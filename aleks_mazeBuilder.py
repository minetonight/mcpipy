...
ppos = Vec3()
ppos.x = x0 - spawnX - mazeXSize/2
ppos.z = z0 - spawnZ - mazeZSize/2
ppos.y = y0 - spawnY
...
def makeWall(x,z):
    mc.setBlock(x,ppos.y,z, material_id)
    mc.setBlock(x,ppos.y+1,z, material_id)
    mc.setBlock(x,ppos.y+2,z, material_id)
...
for x in range(0,mazeXSize-1):
    for z in range(mazeZSize-1):
        mc.setBlock(ppos.x+x,ppos.y,ppos.z+z, material_id)
        for y in range(1,3):
            mc.setBlock(ppos.x+x,ppos.y+y,ppos.z+z, block.AIR)

# Create an empty maze.
initMaze(mazeXSize,mazeZSize)
